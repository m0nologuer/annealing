...
//////////
      ...
      else
      {
        ...
        if (i!=closest_mesh)
          ...
          meshes[i].rotateLessThan(rotation_distance*meshes[closest_mesh].getMass()/total_mass,vector_to_closest_object);

       // assert(!(closest_distance < GAP));
        cout << i << " rotation_distance:" << rotation_distance << endl ;   
      }  
