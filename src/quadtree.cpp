...
Eigen::Vector3d Triangle::shortestDistanceTo(Eigen::Vector3d point){
  ...
  Eigen::Vector3d D = point-points[0];
  Eigen::Vector3d E0 = points[1]-points[0];
  Eigen::Vector3d E1 = points[2]-points[0];

  double a = E0.squaredNorm();
  double b = E0.dot(E1);
  double c = E1.squaredNorm();
  double d = E0.dot(D);
  double e = E1.dot(D);
  ...
  double denom, invDenom, numer, invDet;
  ...
  Eigen::Vector3d closest_point = points[0] + s*E0+ t*E1;
